...
const getActivityService = async () => {
  ...
  return new ActivityService(getDB());
};
...
router.post('/', auth, async (req, res) => {
  try {
    ...
    if (objectId) activityData.object = new ObjectId(objectId);
    if (targetId) activityData.target = new ObjectId(targetId);
    if (images && images.length > 0) activityData.images = images;
